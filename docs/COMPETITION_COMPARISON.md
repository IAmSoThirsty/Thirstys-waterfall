...
| Release and image pipeline | Partial | Release `v1.0.3`, GHCR image digest, hosted CI, security scans, local published-image smoke, local Docker target manifest with service hardening, production TLS proxy config verifier | External target/proxy logs, live TLS certificate evidence, external service hardening, and real OS backend evidence |
...
- Public release, image, and local Docker target manifest evidence now exist for `v1.0.3`.
...
- External/public target-host deployment verification.
