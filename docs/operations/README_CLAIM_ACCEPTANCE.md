...
| Total encryption of all data | Not accepted | Crypto modules and encrypted storage helpers exist; orchestrator status no longer reports `everything_encrypted: True` or accepted encryption tier by default | End-to-end proof for stored state, browser data, logs, telemetry, downloads, and transport paths |
